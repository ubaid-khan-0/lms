<x-app-layout :title="$title">
    ...
    <div id="main-content">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <ul class="metismenu list-unstyled">
                            <li><a href=" {{ route('admin.role.create') }}" class="btn btn-sm btn-outline-primary">Add New</a></li>
                        </ul>
                        @endcan
                    </div>
                </div>
        ...
        {{ $dataTable->scripts() }}
        <script>
            function deleteRole(where_column, where_id, where_table) {
                $('#delete_modal').modal('show');
                $('#delColumn').val(where_column);
                $('#delId').val(where_id);
                $('#delTable').val(where_table);
                let deleteUrl = "{{ route('admin.role.destroy',':id') }}".replace(':id', where_id);
